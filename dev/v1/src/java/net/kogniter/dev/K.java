package net.kogniter.dev;


class K {

    static final String TAHAP_APK = "dev";

    static final String VERSI_APK = "1";

    static final String VERSI_WEB = "1.0";

    static final String URL_METADATA = "https://repoinvoker1.github.io/krediter/dev/v1/metadata.json";

    static final String URL_FOLDER_LOKAL = "https://app.local/kogniter/berkas_lokal/";
    static final String URL_REPO_LOKAL = "https://app.local/kogniter/berkas_lokal/repo/";

    static final String N_VERSI_APK = "versi_apk";
    static final String N_VERSI_WEB = "versi_web";
    static final String N_URL_APK = "url_apk";
    static final String N_URL_WEB = "url_web";

    static final String NAMA_PREF_METADATA = "metadata";
    static final String NAMA_FOLDER_REPO_LOKAL = "repo";

    static final int BATAS_WAKTU_UNDUH = 1000 * 20;

}
